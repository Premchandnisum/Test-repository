...
Then /^I should see the order confirmation page$/ do
  ...
  page_navigated = page.has_content? @expectedOrderConfirmationPageTitle
  if (page_navigated)
...
Then /^I should see profile email address$/ do
  puts "Profile email address entered on payment page: " + @createprofileemailaddress
  ...
  if (@createprofileemailaddress == @orderconfrimationprofileemail)
...
Then /^I should see the order confirmation page URL$/ do
  ...
  if (curr_url.index(@order_confirm_page_url))
